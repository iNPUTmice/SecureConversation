...
public abstract class LocationActivity extends AppCompatActivity implements LocationListener {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        final Boolean dark = PreferenceManager.getDefaultSharedPreferences(getApplicationContext())
    ...
    protected void setupMapView() {
        ...
        if (f.exists() && f.isDirectory() && f.canRead() && f.canWrite() && f.canExecute()) {
            ...
            if (cache.exists() && cache.isDirectory() && cache.canRead() && cache.canWrite() && cache.canExecute()) {
                Log.d(Config.Map.LOGTAG, "Using osmand tile cache at: " + cache.getAbsolutePath());
    ...
    protected void requestLocationUpdates() {
        Log.d(Config.Map.LOGTAG, "Requesting location updates...");
