...
public class ShareWithActivity extends XmppActivity implements XmppConnectionService.OnConversationUpdate {
	...
	private void share() {
		...
		if (share.uuid != null) {
			...
			Account account;
			try {
				account = xmppConnectionService.findAccountByJid(JidHelper.fromString(share.account));
			...
			try {
				conversation = xmppConnectionService
						.findOrCreateConversation(account, JidHelper.fromString(share.contact), false,true);
			} catch (final IllegalArgumentException e) {
				return;
			}
		}
