...
public class XmppConnection implements Runnable {
    ...
    public final OnIqPacketReceived registrationResponseListener = new OnIqPacketReceived() {
        ...
        public void onIqPacketReceived(Account account, IqPacket packet) {
            if (packet.getType() == IqPacket.TYPE.RESULT) {
                ...
                Log.d(Config.LOGTAG, account.getScrambledJid()+": successfully registered new account on server");
    ...
    private void changeStatus(final Account.State nextStatus) {
        synchronized (this) {
            if (Thread.currentThread().isInterrupted()) {
                Log.d(Config.LOGTAG, account.getScrambledJid() + ": not changing status to " + nextStatus + " because thread was interrupted");
                return;
            }
    ...
    protected void connect() {
        ...
        Log.d(Config.LOGTAG, account.getScrambledJid() + ": connecting");
        ...
        try {
            ...
            if (useTor) {
                ...
                Log.d(Config.LOGTAG, account.getScrambledJid() + ": connect to " + destination + " via Tor. directTls="+directTls);
                ...
                try {
                    ...
                    Log.d(Config.LOGTAG, account.getScrambledJid() + ": thread was interrupted before beginning stream");
                    return;
                } catch (Exception e) {
                ...
                if (Thread.currentThread().isInterrupted()) {
                    Log.d(Config.LOGTAG, account.getScrambledJid() + ": Thread was interrupted");
                    return;
                }
                if (results.size() == 0) {
                ...
                if (hardcoded) {
                    ...
                    if (storedBackupResult != null && !results.contains(storedBackupResult)) {
                        ...
                        Log.d(Config.LOGTAG, account.getScrambledJid() + ": loaded backup resolver result from db: " + storedBackupResult);
                    }
                }
                for (Iterator<Resolver.Result> iterator = results.iterator(); iterator.hasNext(); ) {
                    ...
                    try {
                        ...
                        Log.d(Config.LOGTAG, account.getScrambledJid() + ": " + e.getMessage() + "(" + e.getClass().getName() + ")");
                        if (!iterator.hasNext()) {
            ...
            if (!Thread.currentThread().isInterrupted()) {
                ...
                Log.d(Config.LOGTAG, account.getScrambledJid() + ": not force closing socket because thread was interrupted");
            }
        }
    }
    ...
    public void run() {
        synchronized (this) {
            ...
            if (this.mThread.isInterrupted()) {
                Log.d(Config.LOGTAG, account.getScrambledJid() + ": aborting connect because thread was interrupted");
                return;
            }
            forceCloseSocket();
        }
    ...
    private void processStream() throws XmlPullParserException, IOException {
        ...
        while (nextTag != null && !nextTag.isEnd("stream")) {
            if (nextTag.isStart("error")) {
                ...
                Log.d(Config.LOGTAG, account.getScrambledJid() + ": logged in");
                ...
                try {
                    ...
                    synchronized (this.mStanzaQueue) {
                        ...
                        if (serverCount < stanzasSent) {
                            ...
                            Log.d(Config.LOGTAG, account.getScrambledJid() + ": session resumed");
                        }
                ...
                Log.d(Config.LOGTAG, account.getScrambledJid() + ": online with resource " + account.getResource());
                changeStatus(Account.State.ONLINE);
            } else if (nextTag.isStart("r")) {
                tagReader.readElement(nextTag);
                if (Config.EXTENDED_SM_LOGGING) {
                    Log.d(Config.LOGTAG, account.getScrambledJid() + ": acknowledging stanza #" + this.stanzasReceived);
                }
                ...
                synchronized (NotificationService.CATCHUP_LOCK) {
                    if (mWaitingForSmCatchup.compareAndSet(true, false)) {
                        ...
                        Log.d(Config.LOGTAG, account.getScrambledJid() + ": SM catchup complete (messages=" + messageCount + ", pending IQs="+pendingIQs+")");
                ...
                try {
                    ...
                    Log.d(Config.LOGTAG, account.getScrambledJid() + ": server send ack without sequence number");
                }
            } else if (nextTag.isStart("failed")) {
                Element failed = tagReader.readElement(nextTag);
                try {
                    final int serverCount = Integer.parseInt(failed.getAttribute("h"));
                    Log.d(Config.LOGTAG, account.getScrambledJid() + ": resumption failed but server acknowledged stanza #" + serverCount);
                    final boolean acknowledgedMessages;
                    synchronized (this.mStanzaQueue) {
                        acknowledgedMessages = acknowledgeStanzaUpTo(serverCount);
                    }
                    ...
                    Log.d(Config.LOGTAG, account.getScrambledJid() + ": resumption failed");
                }
                resetStreamId();
                sendBindRequest();
    ...
    private boolean acknowledgeStanzaUpTo(int serverCount) {
        ...
        for (int i = 0; i < mStanzaQueue.size(); ++i) {
            if (serverCount >= mStanzaQueue.keyAt(i)) {
                if (Config.EXTENDED_SM_LOGGING) {
                    Log.d(Config.LOGTAG, account.getScrambledJid() + ": server acknowledged stanza #" + mStanzaQueue.keyAt(i));
                }
    ...
    Element processPacket(final Tag currentTag, final int packetType) throws XmlPullParserException, IOException {
        ...
        if (inSmacksSession) {
            ...
            Log.d(Config.LOGTAG, account.getScrambledJid() + ": not counting stanza(" + element.getClass().getSimpleName() + "). Not in smacks session.");
        }
        lastPacketReceived = SystemClock.elapsedRealtime();
    ...
    private void switchOverToTls() throws XmlPullParserException, IOException {
        ...
        sendStartStream();
        Log.d(Config.LOGTAG, account.getScrambledJid() + ": TLS connection established");
        features.encryptionEnabled = true;
        final Tag tag = tagReader.readTag();
        if (tag != null && tag.isStart("stream")) {
    ...
    private SSLSocket upgradeSocketToTls(final Socket socket) throws IOException {
        ...
        if (!tlsFactoryVerifier.verifier.verify(account.getServer(), this.verifiedHostname, sslSocket.getSession())) {
            Log.d(Config.LOGTAG, account.getScrambledJid() + ": TLS certificate verification failed");
    ...
    private void processStreamFeatures(final Tag currentTag) throws XmlPullParserException, IOException {
        ...
        if (this.streamFeatures.hasChild("starttls") && !features.encryptionEnabled) {
            ...
            if (Config.EXTENDED_SM_LOGGING) {
                Log.d(Config.LOGTAG, account.getScrambledJid() + ": resuming after stanza #" + stanzasReceived);
            }
    ...
    private void authenticate() throws IOException {
        ...
        if (saslMechanism != null) {
            ...
            Log.d(Config.LOGTAG, account.getScrambledJid() + ": Authenticating with " + saslMechanism.getMechanism());
    ...
    private void sendBindRequest() {
        try {
            ...
            Log.d(Config.LOGTAG, account.getScrambledJid() + ": interrupted while waiting for DB restore during bind");
            return;
        }
        clearIqCallbacks();
                ...
                if (jid != null && jid.getContent() != null) {
                    try {
                        ...
                        if (!account.getJid().getDomain().equals(assignedJid.getDomain())) {
                            Log.d(Config.LOGTAG, account.getScrambledJid() + ": server tried to re-assign domain to " + assignedJid.getDomain());
                            throw new StateChangingError(Account.State.BIND_FAILURE);
                        }
                        if (account.setJid(assignedJid)) {
                            Log.d(Config.LOGTAG, account.getScrambledJid() + ": jid changed during bind. updating database");
                        ...
                        Log.d(Config.LOGTAG, account.getScrambledJid() + ": server reported invalid jid (" + jid.getContent() + ") on bind");
                    }
                } else {
                    Log.d(Config.LOGTAG, account.getScrambledJid() + ": disconnecting because of bind failure. (no jid)");
                }
            } else {
                Log.d(Config.LOGTAG, account.getScrambledJid() + ": disconnecting because of bind failure (" + packet.toString());
            }
    ...
    private void clearIqCallbacks() {
        ...
        synchronized (this.packetCallbacks) {
            ...
            Log.d(Config.LOGTAG, account.getScrambledJid() + ": clearing " + this.packetCallbacks.size() + " iq callbacks");
        ...
        for (OnIqPacketReceived callback : callbacks) {
            try {
                ...
                Log.d(Config.LOGTAG, account.getScrambledJid() + ": caught StateChangingError(" + error.state.toString() + ") while clearing callbacks");
                //ignore
            }
        }
        Log.d(Config.LOGTAG, account.getScrambledJid() + ": done clearing iq callbacks. " + this.packetCallbacks.size() + " left");
    }

    public void sendDiscoTimeout() {
        if (mWaitForDisco.compareAndSet(true, false)) {
            Log.d(Config.LOGTAG, account.getScrambledJid() + ": finalizing bind after disco timeout");
            finalizeBind();
        }
    }

    private void sendStartSession() {
        Log.d(Config.LOGTAG, account.getScrambledJid() + ": sending legacy session to outdated server");
    ...
    private void sendPostBindInitialization() {
        ...
        Log.d(Config.LOGTAG, account.getScrambledJid() + ": starting service discovery");
        mPendingServiceDiscoveries.set(0);
        if (smVersion == 0 || Patches.DISCO_EXCEPTIONS.contains(account.getJid().getDomain())) {
            Log.d(Config.LOGTAG, account.getScrambledJid() + ": do not wait for service discovery");
        ...
        if (discoveryResult == null) {
            ...
            Log.d(Config.LOGTAG, account.getScrambledJid() + ": server caps came from cache");
    ...
    private void sendServiceDiscoveryInfo(final Jid jid) {
        ...
        this.sendIqPacket(iq, (account, packet) -> {
            if (packet.getType() == IqPacket.TYPE.RESULT) {
                ...
                Log.d(Config.LOGTAG, account.getScrambledJid() + ": could not query disco info for " + jid.toString());
    ...
    private void enableAdvancedStreamFeatures() {
        if (getFeatures().blocking() && !features.blockListRequested) {
            Log.d(Config.LOGTAG, account.getScrambledJid() + ": Requesting block list");
    ...
    private void sendServiceDiscoveryItems(final Jid server) {
        ...
        this.sendIqPacket(iq, (account, packet) -> {
            if (packet.getType() == IqPacket.TYPE.RESULT) {
                ...
                Log.d(Config.LOGTAG, account.getScrambledJid() + ": could not query disco items of " + server);
            }
            if (packet.getType() != IqPacket.TYPE.TIMEOUT) {
                if (mPendingServiceDiscoveries.decrementAndGet() == 0
                        && mWaitForDisco.compareAndSet(true, false)) {
    ...
    private void processStreamError(final Tag currentTag) throws XmlPullParserException, IOException {
        ...
        if (streamError.hasChild("conflict")) {
            account.setResource(createNewResource());
            Log.d(Config.LOGTAG, account.getScrambledJid() + ": switching resource due to conflict (" + account.getResource() + ")");
            ...
            Log.d(Config.LOGTAG, account.getScrambledJid() + ": stream error " + streamError.toString());
    ...
    private synchronized void sendPacket(final AbstractStanza packet, final boolean force) {
        ...
        synchronized (this.mStanzaQueue) {
            if (force || isBound) {
                ...
                Log.d(Config.LOGTAG, account.getScrambledJid() + " do not write stanza to unbound stream " + packet.toString());
            }
            if (packet instanceof AbstractAcknowledgeableStanza) {
                ...
                if (stanza instanceof MessagePacket && stanza.getId() != null && inSmacksSession) {
                    if (Config.EXTENDED_SM_LOGGING) {
                        Log.d(Config.LOGTAG, account.getScrambledJid() + ": requesting ack for message stanza #" + stanzasSent);
                    }
    ...
    public void disconnect(final boolean force) {
        interrupt();
        Log.d(Config.LOGTAG, account.getScrambledJid() + ": disconnecting force=" + force);
        if (force) {
            forceCloseSocket();
        } else {
            ...
            if (currentTagWriter.isActive()) {
                ...
                try {
                    ...
                    Log.d(Config.LOGTAG, account.getScrambledJid() + ": closing stream");
                    ...
                    if (streamCountDownLatch != null) {
                        if (streamCountDownLatch.await(1, TimeUnit.SECONDS)) {
                            Log.d(Config.LOGTAG, account.getScrambledJid() + ": remote ended stream");
                        } else {
                            Log.d(Config.LOGTAG, account.getScrambledJid() + ": remote has not closed socket. force closing");
                        }
                    }
                } catch (InterruptedException e) {
                    Log.d(Config.LOGTAG, account.getScrambledJid() + ": interrupted while gracefully closing stream");
                } catch (final IOException e) {
                    Log.d(Config.LOGTAG, account.getScrambledJid() + ": io exception during disconnect (" + e.getMessage() + ")");
                } finally {
                    ...
                    if (items.size() > 0) {
                        try {
                            ...
                            if (filesize <= maxsize) {
                                ...
                                Log.d(Config.LOGTAG, account.getScrambledJid() + ": http upload is not available for files with size " + filesize + " (max is " + maxsize + ")");
                                return false;
                            }
                        } catch (Exception e) {
                            return true;
